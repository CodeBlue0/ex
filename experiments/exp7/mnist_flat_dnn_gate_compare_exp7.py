...
class TwoGateDNN(nn.Module):
    ...
    def set_feature_stats(self, mean: torch.Tensor, std: torch.Tensor) -> None:
        self.feature_mean.copy_(mean.view(-1))
        self.feature_std.copy_(std.view(-1))

    def forward_with_spaces(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        x = x.view(x.size(0), -1)
        x_norm = (x - self.feature_mean.unsqueeze(0)) / self.feature_std.unsqueeze(0)
        g_a = torch.softmax(self.gate_a_net(x_norm), dim=1)
        g_b = torch.softmax(self.gate_b_net(x_norm), dim=1)
        z_a = g_a * x_norm
        z_b = (-g_b) * x_norm
        # Shared backbone is applied to both branches; gate_b branch uses signed negative input.
        logits_a = self.backbone(z_a)
        logits_b = self.backbone(z_b)
...
def compute_feature_stats(
    ...
    loader = DataLoader(subset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=False)
    sum_x: torch.Tensor | None = None
    sum_x2: torch.Tensor | None = None
    count = 0

    for xb, _ in loader:
        x_flat = xb.view(xb.size(0), -1).float()
        if sum_x is None:
            dim = x_flat.size(1)
        ...
        sum_x += x_flat.sum(dim=0, dtype=torch.float64)
        sum_x2 += (x_flat * x_flat).sum(dim=0, dtype=torch.float64)
        count += int(x_flat.size(0))

    if count <= 0 or sum_x is None or sum_x2 is None:
        raise RuntimeError("failed_to_compute_feature_stats_empty_subset")
...
def main(args: argparse.Namespace) -> None:
    ...
    feat_mean, feat_std = compute_feature_stats(
        ...
        num_workers=args.stat_num_workers,
        eps=args.feature_norm_eps,
    )
    model.set_feature_stats(feat_mean, feat_std)
    print("feature_norm=per_feature_standardize_before_gate")
    print(f"feature_norm_eps={args.feature_norm_eps:.1e}")
    print(f"feature_norm_mean_avg={float(feat_mean.mean().item()):.6f}")
    print(f"feature_norm_std_avg={float(feat_std.mean().item()):.6f}")
    ...
    save_space_tsne_by_pred(g_diff, pred_label, args.gdiff_tsne_out, "g-diff space t-SNE ((g_a - g_b), color=pred label 0/1)")
    ...
    save_space_tsne_by_digit10(g_diff, true_digit, args.gdiff_tsne_out_digit10, "g-diff space t-SNE ((g_a - g_b), color=true digit 0-9)")
    ...
    save_space_tsne_with_sim(
        ...
        anchor_idx,
        pred_label,
        args.gdiff_sim_a_tsne_out,
        f"g-diff space t-SNE (SIM_a from g_a cosine, cos>={thr_a:.4f})",
    )
    save_space_tsne_with_sim(
        z_space,
        sim_a,
    ...
    save_space_tsne_with_sim_digit10(
        ...
        anchor_idx,
        true_digit,
        args.gdiff_sim_a_tsne_out_digit10,
        f"g-diff space t-SNE (SIM_a, true digit 0-9, cos>={thr_a:.4f})",
    )
    save_space_tsne_with_sim_digit10(
        z_space,
        sim_a,
    ...
    save_space_tsne_with_sim(
        ...
        anchor_idx,
        pred_label,
        args.gdiff_sim_b_tsne_out,
        f"g-diff space t-SNE (SIM_b: cosine(g_a, g_b_anchor), cos>={thr_b:.4f})",
    )
    save_space_tsne_with_sim(
        z_space,
        sim_b,
    ...
    save_space_tsne_with_sim_digit10(
        ...
        anchor_idx,
        true_digit,
        args.gdiff_sim_b_tsne_out_digit10,
        f"g-diff space t-SNE (SIM_b, true digit 0-9, cos>={thr_b:.4f})",
    )
    save_space_tsne_with_sim_digit10(
        z_space,
        sim_b,
...
if __name__ == "__main__":
    ...
    default_tag = "exp7_featnorm_twogate_backbone_diff_negb"
    ...
    parser.add_argument("--stat-num-workers", type=int, default=2)
    ...
    parser.add_argument("--feature-norm-eps", type=float, default=1e-6)
    parser.add_argument("--stat-batch-size", type=int, default=2048)
    ...
    parser.add_argument("--gdiff-tsne-out", type=str, default=default_out("mnist_gdiff_space_tsne_pred01"))
    ...
    parser.add_argument("--gdiff-tsne-out-digit10", type=str, default=default_out("mnist_gdiff_space_tsne_digit10"))
    ...
    parser.add_argument("--gdiff-sim-a-tsne-out", type=str, default=default_out("mnist_gdiff_space_tsne_sim_a"))
    ...
    parser.add_argument("--gdiff-sim-a-tsne-out-digit10", type=str, default=default_out("mnist_gdiff_space_tsne_sim_a_digit10"))
    ...
    parser.add_argument("--gdiff-sim-b-tsne-out", type=str, default=default_out("mnist_gdiff_space_tsne_sim_b"))
    ...
    parser.add_argument("--gdiff-sim-b-tsne-out-digit10", type=str, default=default_out("mnist_gdiff_space_tsne_sim_b_digit10"))
