...
class GatedDNN(nn.Module):
    ...
    def forward_with_spaces(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        g = torch.softmax(self.gate_net(x_norm), dim=1)
...
def save_gz_tsne_with_gspace_sim(
    ...
    save_space_tsne_with_sim(
        ...
        label=pred_label,
        out_path=g_out,
        title=f"g-space t-SNE (SIM from g-space cosine, cos>={effective_threshold:.4f})",
    )
    save_space_tsne_with_sim(
        space=z_space,
        sim_mask=sim_mask,
        ...
        label=pred_label,
        out_path=z_out,
        title=f"z-space t-SNE (SIM from g-space cosine, cos>={effective_threshold:.4f})",
    )
    save_space_tsne_with_sim_digit10(
        ...
        title=f"g-space t-SNE (SIM from g-space cosine, true digit 0-9, cos>={effective_threshold:.4f})",
    )
    save_space_tsne_with_sim_digit10(
        space=z_space,
        sim_mask=sim_mask,
        ...
        title=f"z-space t-SNE (SIM from g-space cosine, true digit 0-9, cos>={effective_threshold:.4f})",
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    gate.set_feature_stats(feat_mean, feat_std)
    print(f"feature_norm=per_feature_standardize_before_gate")
    ...
    save_space_tsne_by_pred(
        ...
        title="g-space t-SNE (raw, color=pred 0/1)",
    )
    save_space_tsne_by_pred(
        space=z_space,
        pred_label=pred_label,
    ...
    print("[baseline] skipped in exp3")
...
if __name__ == "__main__":
    ...
    default_key_dims = ""
    default_tag = "exp3_featnorm_rawg"
